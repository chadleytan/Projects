...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        ...
        """
        symbols, symbol_set, n = self._symbols, self._symbol_set, self._n
        #Find all indices where "*" occurs
        empty_indices = [i for i, j in enumerate(symbols) if j == "*"]

        #Checks through all indices with "*"
        for index in empty_indices:
            #Assign the sets of row, column and subsquare
            row = self._row_set(index)
            column = self._column_set(index)
            subsquare = self._subsquare_set(index)
            #Row has a blank space
            if (len(row) == n and row - symbol_set == {"*"}):
                missing = (symbol_set - row).pop()
                if missing in column or missing in subsquare:
                    return True
            #Column has a blank space
            elif (len(column) == n and column - symbol_set == {"*"}):
                missing = (symbol_set - column).pop()
                if missing in row or missing in subsquare:
                    return True
            #Subsquare has a blank space
            elif (len(subsquare) == n and subsquare - symbol_set == {"*"}):
                missing = (symbol_set - subsquare).pop()
                if missing in column or missing in row:
                    return True
        return False
